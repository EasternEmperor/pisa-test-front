<template>
    <div class="big-air-controller-t1">
      <!-- 使用 Header 组件 -->
      <header-component :userName="userName" />

      <!-- 题干部分 -->
      <div class="container-box">
        <big-air-controller-up-component ref="upComponentRef" @applyChanges="handleApply" @resetChanges="handleReset" @control="handleControl"/>
      </div>
      ...
      <div class="container-box question-section">
        <div class="question-text">
          <h3>问题1: 控制器功能</h3>
          <p>
            通过改变滑块并调控，弄清楚每个控制器控制着温度、湿度和风量中的哪些因素。<br/>
            你可以通过"重置"键来重置所有组件。<br/>
            在下方将控制器和你认为其控制的对象连上线。<br/>
            <b>连线的操作是</b>：点击一个控制器方块，然后点击另一个温度、湿度或风量方块。<br/>
        ...
        <div class="diagram-section">
          ...
          <div class="influences-column">
            ...
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'wind' }"
              @click="handleBoxClick('wind')"
            >
              风量
            </div>
          </div>
  ...
  <script>
  import BigAirControllerUpComponent from './big_air_controller_up_component.vue';
  import HeaderComponent from '@/components/Header.vue';
  
  export default {
    name: 'BigAirControllerT1',
    components: {
    ...
    methods: {
      ...
      sendEvent(eventType) {
        ...
        const data = {
          tableName: 1,
          htmlName: 'big_air_controller_t1',
          userName: userName,
          ithAnswer: ithAnswer,
          event: 'ACER_EVENT',
          eventType: eventType,
        ...
        if (eventType === 'control') {
          ...
          data.windValue = upComponent.wind.toString();
      ...
      isInfluence(type) {
        return ['temp', 'humid', 'wind'].includes(type);
      },
      getDiagramState() {
        ...
        const state = sortedConnections.map(conn => {
          ...
          const end = conn.end === 'temp' ? 'temp' : conn.end === 'humid' ? 'humid' : 'wind';
          return `${start}->${end}`;
        }).join(', ');
    ...
    mounted() {
      ...
      this.$el.querySelectorAll('.influence-box').forEach(el => {
        let text = el.textContent.trim();
        text = text.replace('温度', 'temp').replace('湿度', 'humid').replace('风量', 'wind');
