<template>
    <div class="camera-controller-t1">
      <!-- 使用 Header 组件 -->
      <header-component :userName="userName" />

      <!-- 题干部分 -->
      <div class="container-box">
        <camera-controller-up-component ref="upComponentRef" @applyChanges="handleApply" @resetChanges="handleReset" @control="handleControl"/>
      </div>
      ...
      <div class="container-box question-section">
        <div class="question-text">
          <h3>问题1: 控制器功能</h3>
          <p>
            通过改变滑块并调控，弄清楚每个控制器控制着照片亮度、清晰度、虚化程度和取景范围中的哪些因素。<br/>
            你可以通过"重置"键来重置所有组件。<br/>
            在下方将控制器和你认为其控制的对象连上线。<br/>
            <b>连线的操作是</b>：点击一个控制器方块，然后点击另一个亮度、清晰度、虚化程度和取景范围方块。<br/>
        ...
        <div class="diagram-section">
          ...
          <div class="influences-column">
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'brightness' }"
              @click="handleBoxClick('brightness')"
            >
              亮度
            </div>
            <div
            ...
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'virtualization' }"
              @click="handleBoxClick('virtualization')"
            >
              虚化程度
            </div>
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'range' }"
              @click="handleBoxClick('range')"
            >
              取景范围
            </div>
          </div>
  ...
  <script>
  import CameraControllerUpComponent from './camera_controller_up_component.vue';
  import HeaderComponent from '@/components/Header.vue';
  
  export default {
    name: 'CameraControllerT1',
    components: {
      CameraControllerUpComponent,
      HeaderComponent,
    },
    data() {
      return {
    ...
    methods: {
      ...
      isInfluence(type) {
        return ['brightness', 'definition', 'virtualization', 'range'].includes(type);
      },
      getDiagramState() {
        ...
        const state = sortedConnections.map(conn => {
          ...
          const end = conn.end === 'brightness' ? 'brightness' : conn.end === 'definition' ? 'definition' : conn.end === 'virtualization' ? 'virtualization' : 'range';
          return `${start}->${end}`;
        }).join(', ');
    ...
    mounted() {
      ...
      this.$el.querySelectorAll('.influence-box').forEach(el => {
        let text = el.textContent.trim();
        text = text.replace('亮度', 'brightness').replace('清晰度', 'definition').replace('虚化程度', 'virtualization').replace('取景范围', 'range');
