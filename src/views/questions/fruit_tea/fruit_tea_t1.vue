<template>
    <div class="fruit-tea-t1">
      <!-- 使用 Header 组件 -->
      <header-component :userName="userName" />

      <!-- 题干部分 -->
      <div class="container-box">
        <fruit-tea-up-component ref="upComponentRef" @applyChanges="handleApply" @resetChanges="handleReset" @control="handleControl"/>
      </div>
      ...
      <div class="container-box question-section">
        ...
        <div class="question-text">
          <h3>问题1: 控制器功能</h3>
          <p>
            通过改变滑块并调控，弄清楚每个控制器控制着果茶容量、温度、甜度和果肉数量中的哪些因素。<br/>
            你可以通过"重置"键来重置所有组件。<br/>
            在下方将控制器和你认为其控制的对象连上线。<br/>
            <b>连线的操作是</b>：点击一个控制器方块，然后点击另一个容量、温度、甜度和果肉数量方块。<br/>
        ...
        <div class="diagram-section">
          ...
          <div class="influences-column">
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'capacity' }"
              @click="handleBoxClick('capacity')"
            >
              容量
            </div>
            <div
            ...
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'pulp' }"
              @click="handleBoxClick('pulp')"
            >
              果肉数量
            </div>
          </div>
  ...
  <script>
  import FruitTeaUpComponent from './fruit_tea_up_component.vue';
  ...
  export default {
    ...
    methods: {
      ...
      sendEvent(eventType) {
        ...
        if (eventType === 'control') {
          ...
          data.pulpValue = upComponent.pulp.toString();
      ...
      isInfluence(type) {
        return ['capacity', 'temp', 'sweetness', 'pulp'].includes(type);
      },
      getDiagramState() {
        ...
        const state = sortedConnections.map(conn => {
          ...
          const end = conn.end === 'capacity' ? 'capacity' : conn.end === 'temp' ? 'temp' : conn.end === 'sweetness' ? 'sweetness' : 'pulp';
          return `${start}->${end}`;
        }).join(', ');
    ...
    mounted() {
      ...
      this.$el.querySelectorAll('.influence-box').forEach(el => {
        let text = el.textContent.trim();
        text = text.replace('容量', 'capacity').replace('温度', 'temp').replace('甜度', 'sweetness').replace('果肉数量', 'pulp');
