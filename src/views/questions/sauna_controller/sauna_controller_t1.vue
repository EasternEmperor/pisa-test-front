<template>
    <div class="sauna-controller-t1">
      <!-- 使用 Header 组件 -->
      <header-component :userName="userName" />

      <!-- 题干部分 -->
      <div class="container-box">
        <sauna-controller-up-component ref="upComponentRef" @applyChanges="handleApply" @resetChanges="handleReset" @control="handleControl"/>
      </div>
      ...
      <div class="container-box question-section">
        ...
        <div class="question-text">
          <h3>问题1: 控制器功能</h3>
          <p>
            通过改变滑块并调控，弄清楚每个控制器控制着房间温度、房间湿度和桑拿时间中的哪些因素。<br/>
            你可以通过"重置"键来重置所有组件。<br/>
            在下方将控制器和你认为其控制的对象连上线。<br/>
            <b>连线的操作是</b>：点击一个控制器方块，然后点击另一个房间温度、房间湿度或桑拿时间方块。<br/>
        ...
        <div class="diagram-section">
          ...
          <div class="influences-column">
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'temp' }"
              @click="handleBoxClick('temp')"
            >
              房间温度
            </div>
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'humid' }"
              @click="handleBoxClick('humid')"
            >
              房间湿度
            </div>
            <div
  ...
  <script>
  import SaunaControllerUpComponent from './sauna_controller_up_component.vue';
  import HeaderComponent from '@/components/Header.vue';
  import CountdownTimer from '@/components/CountdownTimer.vue';
  
  export default {
    name: 'SaunaControllerT1',
    components: {
      SaunaControllerUpComponent,
      HeaderComponent,
      CountdownTimer,
    },
    data() {
    ...
    methods: {
      ...
      sendEvent(eventType) {
        ...
        if (eventType === 'control') {
          ...
          data.humidValue = upComponent.humid.toString();
      ...
      isInfluence(type) {
        return ['temp', 'humid', 'time'].includes(type);
      },
      getDiagramState() {
        ...
        const state = sortedConnections.map(conn => {
          ...
          const end = conn.end === 'temp' ? 'temp' : conn.end === 'humid' ? 'humid' : 'time';
          return `${start}->${end}`;
        }).join(', ');
    ...
    mounted() {
      ...
      this.$el.querySelectorAll('.influence-box').forEach(el => {
        let text = el.textContent.trim();
        text = text.replace('房间温度', 'temp').replace('房间湿度', 'humid').replace('桑拿时间', 'time');
