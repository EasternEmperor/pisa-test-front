<template>
    <div class="sunlight-controller-t1">
      <!-- 使用 Header 组件 -->
      <header-component :userName="userName" />

      <!-- 题干部分 -->
      <div class="container-box">
        <sunlight-controller-up-component ref="upComponentRef" @applyChanges="handleApply" @resetChanges="handleReset" @control="handleControl"/>
      </div>
      ...
      <div class="container-box question-section">
        ...
        <div class="question-text">
          <h3>问题1: 控制器功能</h3>
          <p>
            通过改变滑块并调控，弄清楚每个控制器控制着照射面积、照射时间和照射强度中的哪些因素。<br/>
            你可以通过"重置"键来重置所有组件。<br/>
            在下方将控制器和你认为其控制的对象连上线。<br/>
            <b>连线的操作是</b>：点击一个控制器方块，然后点击另一个照射面积、照射时间或照射强度方块。<br/>
        ...
        <div class="diagram-section">
          ...
          <div class="influences-column">
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'area' }"
              @click="handleBoxClick('area')"
            >
              照射面积
            </div>
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'time' }"
              @click="handleBoxClick('time')"
            >
              照射时间
            </div>
            <div
              class="influence-box"
              :class="{ 'selected-box': selectedControl === 'strength' }"
              @click="handleBoxClick('strength')"
            >
              照射强度
            </div>
          </div>
  ...
  <script>
  import SunlightControllerUpComponent from './sunlight_controller_up_component.vue';
  import HeaderComponent from '@/components/Header.vue';
  import CountdownTimer from '@/components/CountdownTimer.vue';
  
  export default {
    name: 'SunlightControllerT1',
    components: {
      SunlightControllerUpComponent,
      HeaderComponent,
      CountdownTimer,
    },
    data() {
    ...
    methods: {
      ...
      sendEvent(eventType) {
        ...
        const data = {
          tableName: 1,
          htmlName: 'sunlight_controller_t1',
          userName: userName,
          ithAnswer: ithAnswer,
          event: 'ACER_EVENT',
          eventType: eventType,
        ...
        if (eventType === 'control') {
          ...
          data.timeValue = upComponent.time.toString();
          data.strengthValue = upComponent.strength.toString();
      ...
      isInfluence(type) {
        return ['area', 'time', 'strength'].includes(type);
      },
      getDiagramState() {
        ...
        const state = sortedConnections.map(conn => {
          ...
          const end = conn.end === 'area' ? 'area' : conn.end === 'time' ? 'time' : 'strength';
          return `${start}->${end}`;
        }).join(', ');
    ...
    mounted() {
      ...
      this.$el.querySelectorAll('.influence-box').forEach(el => {
        let text = el.textContent.trim();
        text = text.replace('照射面积', 'area').replace('照射时间', 'time').replace('照射强度', 'strength');
